...
variable "test_environment_id" {
  description = "Orchestrator environment the action runs on."
  type        = string
}
...
resource "aria_orchestrator_action" "test" {
  ...
  lifecycle {
    postcondition {
      condition     = self.environment_id == var.test_environment_id
      error_message = "Environment must be ${var.test_environment_id}, actual ${self.environment_id}"
    }
  }
}
`,
