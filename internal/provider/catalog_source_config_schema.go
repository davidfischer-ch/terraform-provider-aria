	...
	return schema.SingleNestedAttribute{
		...
		Attributes: map[string]schema.Attribute{
			"source_project_id": schema.StringAttribute{
				MarkdownDescription: "Project to make available (required for Cloud Templates or ABX Actions catalog sources)",
				Optional:            true,
				Computed:            true,
				Default:             stringdefault.StaticString(""),
			},
			"workflows": schema.ListNestedAttribute{
				MarkdownDescription: "Workflows to make available (required for Orchestrator Worflows catalog sources)",
				Optional:            true,
